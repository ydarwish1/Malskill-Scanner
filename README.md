...
[eval/BENCHMARK.md](eval/BENCHMARK.md) is generated by `eval/run_bench.py`. It lays every labeled fixture out as a single bundle, a skills repo, a benign-only skills repo, a plugin marketplace and project skills under `.claude/skills/`. A malicious sample counts only when its expected rule fires inside its own directory, and a benign sample is a false positive when anything does.

| Layout | Detected | False positives |
| --- | ---: | ---: |
| Each skill scanned alone | 13/13 | 0/10 |
| Skills repo (`skills/<name>/`) | 13/13 | 0/10 |
| Benign-only skills repo | n/a | 0/10 |
| Plugin marketplace | 13/13 | 0/10 |
| Project skills (`.claude/skills/<name>/`) | 13/13 | 0/10 |

Before repositories were split into one bundle per skill, every multi-skill layout caught 10/13, and the benign-only repo produced a false CRITICAL. On the pinned public corpora in [eval/RESULTS.md](eval/RESULTS.md), [anthropics/skills](https://github.com/anthropics/skills) now scans as 21 bundles instead of 1, with zero findings.

```bash
python3 eval/run_bench.py --out eval/BENCHMARK.md   # offline, about a second
```
