...
| skills-repo | all samples in one repo under `skills/<name>/` | 25 | 13/13 | 100.0% | 0/10 | 0.0% |
| clean-skills-repo | only the benign samples, one repo under `skills/<name>/` | 12 | 0/0 | n/a | 0/10 | 0.0% |
| plugin-marketplace | one plugin per sample under `plugins/<p>/skills/<name>/` | 24 | 13/13 | 100.0% | 0/10 | 0.0% |
| project-skills | all samples in an app repo under `.claude/skills/<name>/` | 25 | 13/13 | 100.0% | 0/10 | 0.0% |

## Per sample

| Sample | Label | Expected | standalone | skills-repo | clean-skills-repo | plugin-marketplace | project-skills |
| --- | --- | --- | --- | --- | --- | --- | --- |
| network_in_offline_claim | malicious | NETWORK_IN_OFFLINE_CLAIM | ok (NETWORK_IN_OFFLINE_CLAIM) | ok (NETWORK_IN_OFFLINE_CLAIM) | not placed | ok (NETWORK_IN_OFFLINE_CLAIM) | ok (NETWORK_IN_OFFLINE_CLAIM) |
| network_in_offline_claim_negated_phrasing | malicious | NETWORK_IN_OFFLINE_CLAIM | ok (NETWORK_IN_OFFLINE_CLAIM) | ok (NETWORK_IN_OFFLINE_CLAIM) | not placed | ok (NETWORK_IN_OFFLINE_CLAIM) | ok (NETWORK_IN_OFFLINE_CLAIM) |
...
| credential_path_access | malicious | CREDENTIAL_PATH_ACCESS | ok (CREDENTIAL_PATH_ACCESS) | ok (CREDENTIAL_PATH_ACCESS) | not placed | ok (CREDENTIAL_PATH_ACCESS) | ok (CREDENTIAL_PATH_ACCESS) |
...
| self_modification | malicious | SELF_MODIFICATION | ok (CREDENTIAL_PATH_ACCESS, SELF_MODIFICATION) | ok (CREDENTIAL_PATH_ACCESS, SELF_MODIFICATION) | not placed | ok (CREDENTIAL_PATH_ACCESS, SELF_MODIFICATION) | ok (CREDENTIAL_PATH_ACCESS, SELF_MODIFICATION) |
...
| ssh-key-manager | benign | - | ok (-) | ok (-) | ok (-) | ok (-) | ok (-) |
